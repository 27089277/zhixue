...
// 建 store 时同步恢复登录会话，避免刷新/深链被弹回登录页或工作台。
const initialSession = readLoginSession();
const initAcct = initialSession ? loginAccounts[initialSession.phone] : undefined;
const initRole: Role = initialSession?.role ?? "teacher";
const initRoleProfiles = initAcct
  ? {
      ...seedRoleProfiles,
      [initRole]: {
        ...seedRoleProfiles[initRole],
        name: initAcct.name,
        scope: initAcct.scope,
        avatar: initAcct.avatar,
      },
    }
  : seedRoleProfiles;
...
export const useStore = create<StoreState>()(
  persist(
    (set, get) => ({
      ...
      role: initRole,
      currentUserPhone: initialSession?.phone ?? "",
      section: "workspace",
      bankView: "questions",
      loginMethod: "sms",
      aiMode: "auto",
      ...
      switchRole: (role) => {
        ...
        navigateTo(defaultPath(role, get().roleProfiles));
      },
      ...
      addPaper: (paper, opts) => {
        if (opts?.persist !== false) persistPaper(paper); // 落库 MySQL；学生私有真题 persist:false 只存本地
        set((s) => {
